...
class BaselineModel(torch.nn.Module):
    def __init__(self, batch_size, seq_len, input_size, dropout=0.2, num_classes=106):
        ...
        self.branches = torch.nn.ModuleList([torch.nn.LSTM(input_size[0], 512, seq_len),
                         torch.nn.LSTM(input_size[1], 512, seq_len),
                         torch.nn.LSTM(input_size[2], 512, seq_len)])
        """
    ...
    def init_hidden(self, batch_size):
        ...
        hid = []
        hid.append((weight.new(14, 14, 512).zero_().to(device),
                  weight.new(14, 14, 512).zero_().to(device)))
        hid.append((weight.new(14, 14, 512).zero_().to(device),
                    weight.new(14, 14, 512).zero_().to(device)))
        hid.append((weight.new(14, 14, 512).zero_().to(device),
                    weight.new(14, 14, 512).zero_().to(device)))
