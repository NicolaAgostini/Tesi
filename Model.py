...
class BaselineModel(torch.nn.Module):
    def __init__(self, batch_size, seq_len, input_size, dropout=0.8, num_classes=106):
        ...
        self.branches = [torch.nn.LSTM(input_size[0], 256, seq_len).cuda(),
                         torch.nn.LSTM(input_size[1], 256, seq_len).cuda(),
                         torch.nn.LSTM(input_size[2], 256, seq_len).cuda()]
        """
        ...
        self.fc = torch.nn.Linear(256*3, num_classes).cuda()  # without seq_len because i want my output on every timestamp from 0 to 2s of observations
    ...
    def forward(self, feat):  # input will be batch_size * sequence length * input_dim
        ...
        for key, value in enumerate(feat):
            x_mod, hidden = self.branches[key](value)  # x_mod has shapes [batch_size, 14, lstm_hidden_size=1024]
            # print(x_mod.size())
            x.append(x_mod.cuda())  # append to a list
            #x.append(x_mod)
